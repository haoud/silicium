...
impl Physical {
    /// The maximum physical address supported by the `x86_64` architecture. This
    /// is because physical addresses are 52 bits wide in the `x86_64` architecture.
    pub const MAX: Physical = Physical(0x000F_FFFF_FFFF_FFFF);

    /// Creates a new `Physical` address
    ///
    /// # Panics
    /// Panics if the given address is larger than the maximum supported physical
    /// address on the `x86_64` architecture, which is represented by [`Physical::MAX`].
    #[must_use]
    pub const fn new(addr: usize) -> Self {
        match Self::try_new(addr) {
            None => panic!("Physical address is not valid (must be 52 bits wide or less)"),
            Some(addr) => addr,
        }
    }

    /// Creates a new `Physical` address if the given address is valid. Otherwise,
    /// returns `None`.
    #[must_use]
    ...
    /// Create a new `Physical` address without checking if the address is valid.
    ///
    /// # Safety
    ...
    /// by [`Physical::MAX`]. Otherwise, the behavior is undefined.
    #[must_use]
