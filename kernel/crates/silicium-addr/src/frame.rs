...
/// is a newtype around `Physical` that represents a frame in the `x86_64`
/// architecture, and enforce that the address is page-aligned. On the `x86_64`
/// architecture, the default page size is 4096 bytes, so a frame address must
...
impl Frame {
    ...
    ///
    /// # Panics
    /// Panics if the given address is not page-aligned or is larger than the
    ...
    pub const fn new(addr: usize) -> Self {
        match Self::try_new(addr) {
            None => {
                panic!("Frame address must be 52 bits wide or less and page-aligned")
            }
            Some(addr) => addr,
        }
    }

    /// Creates a new `Frame` address if the given address is valid and
    /// page-aligned. Otherwise, returns `None`.
    #[must_use]
    ...
    /// maximum supported physical address on the `x86_64` architecture, which
    /// is represented by [`Physical::MAX`], and that the address is page
    /// aligned. Otherwise, the behavior is undefined.
    #[must_use]
    pub const unsafe fn new_unchecked(addr: usize) -> Self {
        Frame(Physical::new_unchecked(addr))
    }

    /// Creates a new `Frame` from the given frame index. This desugars to
    /// shifting the index to the left by the page shift value, which is 12
    /// on the `x86_64` architecture.
    ///
    /// # Panics
    /// Panics if the given index is larger than the maximum supported frame
    /// index on the `x86_64` architecture, which is represented by
    /// [`Frame::MAX_INDEX`].
    #[must_use]
    ...
    ///
    /// # Panics
    /// Panics if the given pointer is not page-aligned or is larger than the
    /// maximum supported physical address on the `x86_64` architecture, which
    /// is represented by [`Physical::MAX`].
    #[must_use]
    ...
    /// Create a new `Frame` from the given pointer without checking if the
    /// pointer is valid and page-aligned.
    ///
    /// # Safety
    /// The caller must ensure that the given pointer is valid and
    /// page-aligned. Otherwise, the behavior is undefined.
    #[must_use]
    ...
    /// which starts at 0 from frame located at address 0x0 and increments by
    /// 1 for each frame.
    #[must_use]
