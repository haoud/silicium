...
impl Virtual {
    ...
    ///
    /// # Panics
    /// Panics if the given address is not canonical, which means that the most
    /// significant 16 bits of the address must be either all ones or all zeros.
    /// Put more simply, the address can't be between `0x0000_8000_0000_0000` and
    /// `0xFFFF_7FFF_FFFF_FFFF` (inclusive).
    #[must_use]
    ...
    /// Creates a new `Virtual` address if the given address is canonical. Returns
    /// `None` if the address is not canonical.
    ///
    /// For an definition of what is a canonical address, see the documentation for
    /// the [`new`] method.
    #[must_use]
    ...
    /// Create a new `Virtual` address without checking if the address is canonical.
    ///
    /// # Safety
    /// The caller must ensure that the given address is canonical. For an definition
    /// of what is a canonical address, see the documentation for the [`new`] method.
    #[must_use]
    ...
    ///
    /// # Panics
    /// Panics if the given pointer is not canonical. For an definition of what is a
    /// canonical address, see the documentation for the [`new`] method.
    #[must_use]
    pub fn from_ptr<T>(ptr: *const T) -> Self {
        Self::new(ptr as usize)
    }

    /// Creates a new `Virtual` address from a pointer without checking if the address
    /// is canonical.
    ///
    /// # Safety
    /// The caller must ensure that the given pointer is canonical. For an definition
    /// of what is a canonical address, see the documentation for the [`new`] method.
    /// If the provided pointer point to a valid object, then the address should be
    /// canonical. In practice, this method should be safe to use in most cases.
    #[must_use]
