...
bitflags::bitflags! {
    ...
    pub struct Flags: u8 {
        /// If set, the frame is used by the kernel, otherwise it is used by userspace.
        /// This is used to track which frames are used by the kernel.
        const KERNEL = 1 << 0;
    }
}
...
impl Allocator {
    ...
    /// Allocates a free frame from the frame state. Returns `None` if no frame is available, or a
    /// frame if a frame was successfully allocated.
    #[must_use]
    ...
    #[must_use]
    pub fn allocate_range(&mut self, count: usize, flags: Flags) -> Option<Frame> {
        let mut state = STATE.lock();
        let frames = state.frames_info_mut();

        // Find the first range of contiguous free frames. If there is no such range,
        // return `None`
        let index = frames
            .windows(count)
            .enumerate()
    ...
    ///
    /// # Safety
    /// The caller must ensure that the frame is not used by the kernel or any other part of the
    /// system, and that the frame was not already deallocated. Failure to do so will cause undefined
    /// behavior.
    ...
    ///
    /// # Safety
    /// The caller must ensure that the range of frames is not used by the kernel or any other
    ...
    /// This is used to prevent the frame from being deallocated while it is in use: the
    /// frame will need one more call to [`deallocate`] or [`deallocate_range`] to be
    /// effectively deallocated.
    ///
    /// # Safety
    /// The frame must have been previously allocated by [`allocate`] or [`allocate_range`], and
    /// must not have been deallocated. Failure to do so will cause undefined behavior.
    pub unsafe fn reference(&mut self, frame: Frame) {
        self.reference_range(frame, 1);
    }

    /// Retains a range of frames, incrementing the reference count of each frame.
    /// This is used to prevent the frame from being deallocated while it is in use: the
    /// frames will need one more call to [`deallocate`] or [`deallocate_range`] to be
    /// effectively deallocated.
    ///
    /// # Safety
    /// The range of frame must have been previously allocated by [`allocate_range`] or
    /// [`allocate`], and must not have been deallocated.
    /// Failure to do so will cause undefined behavior.
