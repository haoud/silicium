...
/// This flag is used to prevent the kernel from panicking multiple times in a row. This often
/// means that the kernel has panicked in the panic handler itself, indicating a critical error
/// during the panic handling process.
...
pub fn panic(info: &core::panic::PanicInfo) -> ! {
    // If we have already panicked, we should not panic again because it means that
    // we have panicked in the panic handler itself.
