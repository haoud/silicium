...
/// The kernel PML4, used to map the kernel address space. This PML4 is only
/// modified during the kernel initialization and is never modified after that.
/// It allows us to free the memory used by the bootloader and to create new
/// PML4 extremely fast by simply copying the kernel space of this PML4 into
/// the new one.
...
/// function has been called. Failure to do so will result in undefined
/// behavior, likely to cause a triple fault and a system reboot.
#[init]
pub unsafe fn load_kernel_pml4() {
    KERNEL_PML4.set_current();
}

/// Map the given virtual address to the given frame in the given PML4, with
/// the given flags.
///
/// # Errors
...
///
/// # Safety
/// The caller must ensure that the PML4 is valid and that each frame pointing
...
pub unsafe fn translate<T: addr::virt::Type>(
    ...
    Some(Frame::new(match level {
        ...
        table::Level::Pdpt => {
            usize::from(base) + (usize::from(addr) & 0x3FF_FFFF)
        }
        _ => unreachable!(),
    }))
}
