...
extern "C" {
    /// The interrupt handlers table. This table is defined in the
    /// `interrupt.asm`, and this symbol is used to get the address
    /// of the table in the Rust code.
    static interrupt_handlers: [usize; 0];
}

/// The Interrupt Descriptor Table (IDT) used by the kernel. This is an
/// uninitialized table that will be filled with valid descriptors during
...
pub struct Descriptor {
    ...
    /// gate with a privilege level of 0, meaning that only the kernel can
    /// manually trigger the interrupt (interrupts triggers by the hardware
    /// will still work if it happens in user mode) and interrupt handler will
...
impl Descriptor {
    /// Create a new IDT descriptor with the provided handler address. It will
    /// set the segment selector to 0x08, the flags to 0x8E00 (interrupt gate
    /// with a privilege level of 0) and the handler address to the provided
    /// address.
    #[must_use]
...
#[repr(C, packed)]
pub struct Register {
    /// The size of the IDT in bytes minus one (the limit is inclusive).
    limit: u16,
...
impl Register {
    /// Creates a new uninitialized IDT register. Loading this IDT register in
    /// the CPU will result in a general protection fault, and a triple fault
    /// and a reboot of the computer if the CPU doesn't have a proper exception
    /// handler set up.
    #[must_use]
    pub const fn uninitialized() -> Self {
        Self { limit: 0, base: 0 }
    ...
    /// Creates a new IDT register with the provided table. It will set the
    ...
    ///
    /// # Safety
    /// The caller must ensure that the IDT provided is valid and will remain
...
#[inline]
pub fn setup() {
    // SAFETY: This is safe because we are initializing the IDT table with
    // valid descriptors. Since we use static mut, we must ensure that we
    // have exclusive access to the IDT table (that is the case during the
    // boot process where only one CPU are active) and that we doesn't make
    // multiple mutable references to the IDT table.
    unsafe {
        // The interrupt handlers table is defined in the `asm/interrupt.asm`
        // file. It is a table of simple code stubs that will be called by the
        // CPU when an interrupt is triggered. Each stub has a size of 16
        // bytes, and the address of the first stub is the address of the
        // `interrupt_handlers` symbol.
