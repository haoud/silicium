...
///
/// # Safety
/// The caller must ensure that the address is properly aligned to the type
/// `T`, and that the address is not aliased anywhere in the kernel. Failure
...
/// Obtain a reference to the object located at the given physical address
/// during the execution of the given closure.
///
/// # Safety
/// The caller must ensure that the address is properly aligned to the type
/// `T`, and that the address is not mutably aliased in the kernel. Failure
/// to do so will result in undefined behavior !
#[inline]
...
/// Create a static mutable reference to the object located at the given
/// physical address.
///
...
/// Create a static mutable reference to the slice located at the given
/// physical address with `count` elements.
///
/// # Safety
/// - The physical memory range should be valid and free to use.
...
/// Initialize an object at the given physical address with `obj` and
/// create a static mutable reference to it.
///
/// # Safety
/// - The physical memory range should be valid and free to use.
/// - The physical memory start address should be properly aligned to
...
/// - After this function call, the physical memory range can only be
///   referenced through the returned reference. If the reference is lost,
///   the physical range will be leaked and should never be used again.
///   Failure to do so will result in undefined behavior !
#[inline]
#[must_use]
pub unsafe fn init_and_leak<T>(
    start: impl Into<Physical>,
    obj: T,
) -> &'static mut T {
...
/// Initialize a slice at the given physical address with `obj` for each
/// `count` elements, and create a static mutable reference to it.
///
/// # Safety
/// - The physical memory range should be valid and free to use.
/// - The physical memory start address should be properly aligned to
...
/// - After this function call, the physical memory range can only be
///   referenced through the returned reference. If the reference is lost,
///   the physical memory range will be leaked and should never be used again.
#[inline]
#[must_use]
...
/// Map a physical frame to a virtual address during the execution of the
/// given closure. The caller is responsible for ensuring that the virtual
...
/// Translate a physical address to a virtual address. On this architecture,
/// the translation is very simple since the kernel uses a direct mapping of
/// all the physical memory, mapped to an fixed virtual address.
///
/// However, the caller must ensure to correctly use the returned virtual
...
pub fn translate(phys: impl Into<Physical>) -> Virtual {
    ...
    // will always result in a valid canonical address.
    unsafe {
        Virtual::new_unchecked(
            usize::from(HHDM_START) + usize::from(phys.into()),
        )
    }
}
